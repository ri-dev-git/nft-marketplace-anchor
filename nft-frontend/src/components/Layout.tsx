    ...
    return (
        <div className="flex flex-col h-screen w-screen bg-base-100 text-base-content">
                ...
                <div className="flex items-center gap-3">
                    ...
                    {screenSize === 'tablet' && (
                        <button
                            ...
                            className="p-2 rounded-md hover:bg-base-300 transition-colors sm:block md:block lg:hidden"
                    ...
                    <Link href="/">
                        <h1 className="text-lg sm:text-xl lg:text-2xl font-bold">NFT Market</h1>
                    </Link>
                </div>
            ...
            <div className="flex flex-1 overflow-hidden relative">
                        ...
                        <nav className="flex flex-col place-content-evenly w-80 h-30">
                            {navigationItems.map((item) => (
                                <Link
                                    key={item.href}
                                    href={item.href}
                                    className={`block px-4 py-2 rounded text-lg font-medium hover:bg-base-300 transition-colors ${pathname === item.href ? 'bg-primary text-primary-content' : ''
                                        }`}
                                >
                                    <span className="mr-2">{item.icon}</span>
                                    {item.label}
                ...
                {/* Main Content */}
                <main className={`flex h-full bg-base-100 overflow-hidden ${screenSize === 'desktop' ? 'w-3/4' : 'w-full'
                    }`}>
                    {children}
                </main>
            </div>
